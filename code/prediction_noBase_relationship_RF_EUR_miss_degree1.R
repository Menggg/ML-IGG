    ...
    for(p in rest){
      ...
      for(j in err){
        ...
        for(i in 1:rep){
          ...
          for(r in 1:cycles){
            test=s[,r]
	  index=c(which(x[-test,]$pedigree.relationship=="PO"),which(x[-test,]$pedigree.relationship=="FS"))
	  temp=x[-test,][index,]
...
write.table(table,paste("EUR_miss",miss,"_model_selection_RF_table_relationship_degree1_err",err,".",fam,"fam.txt",sep=''),quote=F,row.names = F,col.names = T)
