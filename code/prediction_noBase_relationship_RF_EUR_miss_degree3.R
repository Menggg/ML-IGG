    ...
    for(p in rest){
      ...
      for(j in err){
        ...
        for(i in 1:rep){
          ...
          for(r in 1:cycles){
            test=s[,r]
	  index=c(which(x[-test,]$pedigree.relationship=="GGP"),which(x[-test,]$pedigree.relationship=="Great-uncle"),which(x[-test,]$pedigree.relationship=="First-cousin"),which(x[-test,]$pedigree.relationship=="Half-uncle"))
          temp=x[-test,][index,]
...
for(i in 1:17)
colnames(table)[i]=paste("top_feature_index_step_",i,sep='')
write.table(table,paste("EUR_miss",miss,"_model_selection_RF_table_relationship_degree3_err",err,".",fam,"fam.txt",sep=''),quote=F,row.names = F,col.names = T)
